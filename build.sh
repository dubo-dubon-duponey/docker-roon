...
. "$(cd "$(dirname "${BASH_SOURCE[0]:-$PWD}")" 2>/dev/null 1>&2 && pwd && cd - >/dev/null)/helpers.sh" --target runtime-bridge
...
# shellcheck source=/dev/null
. "$(cd "$(dirname "${BASH_SOURCE[0]:-$PWD}")" 2>/dev/null 1>&2 && pwd && cd - >/dev/null)/helpers.sh" --target runtime-server
